...
Return only the structured recipe.`;
  const body={model:process.env.OPENAI_MODEL||"gpt-5-mini",store:false,reasoning:{effort:"minimal"},max_output_tokens:2600,input:prompt,text:{verbosity:"low",format:{type:"json_schema",name:"family_recipe",strict:true,schema:RECIPE_SCHEMA}}};
  ...
  const raw=await r.text();let d;try{d=JSON.parse(raw)}catch{throw new Error(`OpenAI recipe returned non-JSON (${r.status}).`)}
  if(!r.ok)throw new Error(d?.error?.message||"OpenAI recipe request failed.");
  const text=outputText(d);if(!text)throw new Error("OpenAI returned no recipe text.");
  return out(200,JSON.parse(text));
